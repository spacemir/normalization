...
##########################################
# 3_0 + noise to basket cell population, 
# recurrent connections for basket-cell pop
...
class HC:
	...
	#rates = np.concatenate((np.arange(0, 1000, 50, "float"),np.arange(1000, 4001, 250, "float")),0)
	...
	def connect(self):
		if not self.built:
			self.build()
		CopyModel("static_synapse", "pyr_excitatory", {"weight":self.params["epsc_pyr_pyr"], \
				  "delay":self.params["d"]})
		...
		CopyModel("static_synapse", "bas_excitatory", {"weight":self.params["epsc_pyr_bas"],\
				  "delay":self.params["d"]})
		CopyModel("static_synapse", "external", {"weight":self.params["epsc_ext_pyr"], \
		          "delay":self.params["d"]})
		...
		for i,mc in enumerate(self.minicolumns):
			...
			RandomConvergentConnect(mc, mc, int(self.params["p_pyr_pyr"]*self.params["N_E"]), \
					model="pyr_excitatory", options={'allow_multapses':self.params["mult"], 'allow_autapses':self.params["aut"]})
			RandomConvergentConnect(mc, self.basket_cells, int(self.params["p_pyr_bas"]*self.params["N_E"]), \
					model="bas_excitatory", options={'allow_multapses':self.params["mult"]})
			RandomConvergentConnect(self.basket_cells, mc, int(self.params["p_bas_pyr"]*self.N_I), \
					model="pyr_inhibitory", options={'allow_multapses':self.params["mult"]})
			RandomDivergentConnect([self.inputs[i]], self.basket_cells, int(self.params["p_in_bas"]*self.N_I), \
					model="bas_excitatory", options={'allow_multapses':self.params["mult"]})
			RandomDivergentConnect([self.inputs[i]], mc, int(self.params["p_in_pyr"]*self.params["N_E"]),\
					model="external",options={'allow_multapses':self.params["mult"]})
		...
		RandomConvergentConnect(self.basket_cells, self.basket_cells, int(self.params["p_bas_bas"]*self.N_I),\
								self.params["ipsc_bas_bas"], self.params["d"], options={'allow_multapses':self.params["mult"],\
							    'allow_autapses':self.params["aut"]})
