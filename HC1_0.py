...
class HC:
	...
	ipsc_recurrent = -500.0		# [pA] amplitude inh. postsynaptic current
	epsc_external = 1.0		#
	t_ref = 10.0
	...
	def connect(self):
		...
		#connect n cells from each minicolumn to voltmeter
		for i,mc in enumerate(self.minicolumns):
			ex_cells = mc[0:HC.n_cells_per_vm]
			ConvergentConnect([self.voltmeters[i]], ex_cells)
		
		#connect n basket cells to voltmeter
	...
	def simulation_info(self):	
		info = ""
		info+= "--------------------------------------\n"
		info+= "HYPERCOLUMNS SIMULATION\t\t" + self.date_time + "\n"
		info+= "--------------------------------------\n"
